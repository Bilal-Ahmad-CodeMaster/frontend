...
function getStrength(pw: string) {
  ...
  return {
    ...
    color: ['', '#f87171', '#fb923c', '#facc15', '#22c55e'][s],
  }
}
...
function Inp({ hasError, className = '', ...p }: React.InputHTMLAttributes<HTMLInputElement> & { hasError?: boolean }) {
  return (
    <input
      {...p}
      className={`w-full px-3.5 py-3 rounded-[10px] bg-[#141920] border text-[13px] font-['Sora'] text-[#EEF2F7] placeholder-[#3D4855] outline-none transition-all
        ${hasError
...
function Alert({ type, msg }: { type: 'error' | 'success'; msg: string }) {
  if (!msg) return null
  return (
    <div className={`flex items-center gap-2 px-3.5 py-2.5 rounded-[10px] text-[12px] font-medium
      ${type === 'error'
...
function SecLabel({ text }: { text: string }) {
  return (
    <p className="text-[11px] font-semibold tracking-widest uppercase text-[#3D4855] border-b border-[#1E2530] pb-2 mt-1">
      {text}
    </p>
  )
}
...
export default function LoginPage() {
  ...
  const switchTab = (t: 'login' | 'register') => {
    setTab(t); setError(''); setSuccess(''); setLogErr({}); setRegErr({})
  }

  const updL = (k: keyof LoginForm, v: string) => {
    setLog(p => ({ ...p, [k]: v }))
    setLogErr(p => ({ ...p, [k]: '' }))
  }

  const updR = (k: keyof RegisterForm, v: string | boolean) => {
    setReg(p => ({ ...p, [k]: v }))
    setRegErr(p => ({ ...p, [k]: '' }))
  }
  ...
  // ── REGISTER ──────────────────────────────────────────────────────────────
  // ✅ FIX: this was the root cause — form was missing onSubmit or button type
  ...
  return (
    <div
      className="min-h-screen bg-[#080B0F] flex items-center justify-center p-4 font-['Sora']"
      style={{
    ...
    >
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Sora:wght@300;400;500;600;700;800&family=Noto+Nastaliq+Urdu:wght@400;700&display=swap');
        @keyframes expand { 0%{transform:scale(.85);opacity:.7} 100%{transform:scale(1.15);opacity:0} }
      `}</style>
      ...
      <div className="flex w-full max-w-[900px] rounded-2xl border border-[#1E2530] overflow-hidden shadow-[0_40px_120px_rgba(0,0,0,0.6)]">
        ...
        <div className="hidden md:flex w-[300px] flex-shrink-0 flex-col items-center justify-center px-8 py-12 bg-gradient-to-br from-[#0e1520] via-[#080B0F] to-[#0d0608] border-r border-[#1E2530] relative overflow-hidden">
          {[180, 280, 400].map((s, i) => (
            <div key={i} className="absolute rounded-full border border-red-500/15"
              style={{ width: s, height: s, animation: `expand 4s ease-out ${i * 1.2}s infinite` }} />
          ))}
          ...
          <p className="mt-1.5 z-10 text-[20px] text-[#6B7685]"
            style={{ fontFamily: "'Noto Nastaliq Urdu',serif", direction: 'rtl' }}>مدد</p>
          <p className="mt-3.5 text-[12px] text-[#6B7685] text-center leading-relaxed max-w-[200px] z-10">
            Voice-first emergency response.<br />Speak. We listen. Help arrives.
          </p>
        ...
        {/* ── RIGHT FORM ── */}
        <div className="flex-1 flex flex-col bg-[#0F1318] overflow-y-auto max-h-[90vh]">
          ...
          {tab === 'login' && (
            <form onSubmit={handleLogin} className="flex flex-col gap-4 p-8">
              ...
              <Field label="Email Address" error={logErr.email}>
                <Inp type="email" placeholder="ali@example.com" hasError={!!logErr.email}
                  value={log.email} onChange={e => updL('email', e.target.value)} />
              </Field>

              <Field label="Password" error={logErr.password}>
                <div className="relative">
                  <Inp type={showPw ? 'text' : 'password'} placeholder="Your password"
                    hasError={!!logErr.password} className="pr-10"
                    value={log.password} onChange={e => updL('password', e.target.value)} />
                  <button type="button" onClick={() => setShowPw(p => !p)}
                    className="absolute right-3 top-1/2 -translate-y-1/2 text-[#6B7685] hover:text-white text-sm">
                    {showPw ? '🙈' : '👁'}
                  </button>
                </div>
              </Field>
              ...
              <button type="submit" disabled={loading}
                className="w-full py-3.5 bg-[#E63946] hover:bg-[#c0303c] disabled:bg-[#3D4855] text-white font-bold text-[13px] tracking-wider rounded-[10px] transition-all shadow-[0_4px_20px_rgba(230,57,70,0.3)] disabled:cursor-not-allowed">
            ...
            <form onSubmit={handleRegister} className="flex flex-col gap-4 p-8">
              ...
              <div className="grid grid-cols-2 gap-3">
                <Field label="First Name" error={regErr.firstName}>
                  <Inp placeholder="Ali" hasError={!!regErr.firstName}
                    value={reg.firstName} onChange={e => updR('firstName', e.target.value)} />
                </Field>
                <Field label="Last Name" error={regErr.lastName}>
                  <Inp placeholder="Hassan" hasError={!!regErr.lastName}
                    value={reg.lastName} onChange={e => updR('lastName', e.target.value)} />
                </Field>
              </div>

              <Field label="Email Address" error={regErr.email}>
                <Inp type="email" placeholder="ali@example.com" hasError={!!regErr.email}
                  value={reg.email} onChange={e => updR('email', e.target.value)} />
              </Field>

              <SecLabel text="Medical Information" />

              <Field label="Pakistan Phone Number" error={regErr.phone}>
                <div className="flex gap-2">
                  <div className="w-[70px] flex-shrink-0 px-3 py-3 bg-red-500/8 border border-red-500/20 rounded-[10px] text-[#E63946] font-bold text-[13px] text-center">
                    +92
                  </div>
                  <Inp placeholder="3001234567" maxLength={10} hasError={!!regErr.phone}
                    value={reg.phone} onChange={e => updR('phone', e.target.value.replace(/\D/, ''))} />
                </div>
              </Field>

              <Field label="Blood Group" error={regErr.bloodGroup}>
                <select value={reg.bloodGroup} onChange={e => updR('bloodGroup', e.target.value)}
                  className={`w-full px-3.5 py-3 rounded-[10px] bg-[#141920] border text-[13px] font-['Sora'] text-[#EEF2F7] outline-none appearance-none transition-all
                    ${regErr.bloodGroup
                      ? 'border-red-400'
              ...
              <SecLabel text="Emergency Contact" />

              <div className="grid grid-cols-2 gap-3">
                <Field label="Contact Name" error={regErr.ecName}>
                  <Inp placeholder="Parent / Spouse" hasError={!!regErr.ecName}
                    value={reg.ecName} onChange={e => updR('ecName', e.target.value)} />
                </Field>
                <Field label="Contact Phone" error={regErr.ecPhone}>
                  <Inp type="tel" placeholder="03XX XXXXXXX" hasError={!!regErr.ecPhone}
                    value={reg.ecPhone} onChange={e => updR('ecPhone', e.target.value)} />
                </Field>
              </div>

              <SecLabel text="Account Security" />

              <Field label="Password" error={regErr.password}>
                <div className="relative">
                  <Inp type={showPw ? 'text' : 'password'} placeholder="Min 8 characters"
                    hasError={!!regErr.password} className="pr-10"
                    value={reg.password} onChange={e => updR('password', e.target.value)} />
                  <button type="button" onClick={() => setShowPw(p => !p)}
                    className="absolute right-3 top-1/2 -translate-y-1/2 text-[#6B7685] hover:text-white text-sm">
                    {showPw ? '🙈' : '👁'}
                  </button>
                    ...
                    <div className="flex gap-1 mt-1.5">
                      {[1, 2, 3, 4].map(i => (
                        <div key={i} className="h-[3px] flex-1 rounded-full transition-all duration-300"
                          style={{ background: i <= pw.score ? pw.color : '#1E2530' }} />
                      ))}
                    </div>
              ...
              <Field label="Confirm Password" error={regErr.confirmPassword}>
                <div className="relative">
                  <Inp type={showCpw ? 'text' : 'password'} placeholder="Repeat password"
                    hasError={!!regErr.confirmPassword} className="pr-10"
                    value={reg.confirmPassword} onChange={e => updR('confirmPassword', e.target.value)} />
              ...
              <label className="flex items-start gap-2.5 cursor-pointer">
                <input type="checkbox" checked={reg.agreed}
                  onChange={e => updR('agreed', e.target.checked)}
                ...
                <span className="text-[11px] text-[#6B7685] leading-relaxed">
                  ...
                  Medical information will be shared with emergency responders.
                </span>
              </label>

              {/* ✅ type="submit" is required — this was the bug */}
              <button type="submit" disabled={loading}
                className="w-full py-3.5 bg-[#E63946] hover:bg-[#c0303c] disabled:bg-[#3D4855] text-white font-bold text-[13px] tracking-wider rounded-[10px] transition-all shadow-[0_4px_20px_rgba(230,57,70,0.3)] disabled:cursor-not-allowed mt-1">
